...
export class Sockets {
  ...
  connect(): void {
    if (!this.socket.connected) {
      ...
      try {
        ...
        console.error("[v0] Failed to connect to socket server:", error)
      }
    } else {
  ...
  sendMessage(
    ...
    console.log("[v0] Sending message with payload:", payload)
    console.log("[v0] Socket connected:", this.socket.connected)
    console.log("[v0] Socket ID:", this.socket.id)

    this.socket.emit("send-message", payload, (response: any) => {
      if (response && response.success) {
        console.log("[v0] Message sent successfully:", response.message)
      } else {
        console.error("[v0] Message send failed:", response?.error || "Unknown error")
      }
    })
  }
